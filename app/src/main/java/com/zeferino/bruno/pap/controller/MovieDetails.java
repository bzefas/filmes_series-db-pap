...
import com.zeferino.bruno.pap.adapter.MovieDetailsSlideAdapter;
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (currentId != this.getArguments().getInt("id") || this.timeOut == 1) {
            ...
            new Thread(new Runnable() {
                public void run() {
                    try {
                        request.execute(Constants.url + "movie/" + currentId + "?append_to_response=releases%2Ctrailers%2Ccasts%2Cimages%2Csimilar&api_key=" + Constants.key).get(10000, TimeUnit.MILLISECONDS);
    ...
    public class onTrailerIconClick implements View.OnClickListener {
        ...
        public void onClick(View v) {
            if (trailerList.size() == 1)
                startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(Constants.youtube + trailerList.get(0))));
            else {
                if (activity.getSearchViewCount())
                    activity.incSearchMovieDetails();
                setAddToBackStack(true);

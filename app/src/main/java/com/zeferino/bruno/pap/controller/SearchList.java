...
import com.zeferino.bruno.pap.adapter.SearchAdapter;
import com.zeferino.bruno.pap.model.MovieModel;
import com.zeferino.bruno.pap.model.SearchModel;
                    ...
                    for (int i = 0; i < searchResultsArray.length(); i++) {
                        ...
                        if (isAdded()) {
                            if (object.has("profile_path") && !object.getString("profile_path").equals("null") && !object.getString("profile_path").isEmpty())
                                movie.setPosterPath(Constants.imageUrl + getResources().getString(R.string.imageSize) + object.getString("profile_path"));
                        }
            ...
            if (loading) {
                if (currentPage != totalPages) {
                    ...
                    new Thread(new Runnable() {
                        public void run() {
                            try {
                                request.execute(Constants.url + "search/multi?query=" + getQuery() + "?&api_key=" + Constants.key + "&page=" + currentPage).get(10000, TimeUnit.MILLISECONDS);
    ...
    public void search() {
        if (getActivity() != null) {
            ...
            new Thread(new Runnable() {
                public void run() {
                    try {
                        request.execute(Constants.url + "search/multi?query=" + getQuery() + "?&api_key=" + Constants.key).get(10000, TimeUnit.MILLISECONDS);
                    } catch (TimeoutException | ExecutionException | InterruptedException e) {
                        request.cancel(true);
                        // we abort the http request, else it will cause problems and slow connection later
                        if (conn != null)

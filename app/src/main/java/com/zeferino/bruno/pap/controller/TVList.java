...
import com.zeferino.bruno.pap.adapter.MovieAdapter;
            ...
            if (loading) {
                if (currentPage != totalPages) {
                    ...
                    new Thread(new Runnable() {
                        public void run() {
                            try {
                                request.execute(Constants.url + getCurrentList() + "?&api_key=" + Constants.key + "&page=" + currentPage).get(10000, TimeUnit.MILLISECONDS);
        ...
        if (listView != null) {
            ...
            new Thread(new Runnable() {
                public void run() {
                    try {
                        if (!isLoading)
                            request.execute(Constants.url + getCurrentList() + "?&api_key=" + Constants.key).get(10000, TimeUnit.MILLISECONDS);
                    } catch (TimeoutException | ExecutionException | InterruptedException e) {
                        request.cancel(true);
                        toastLoadingMore.cancel();
                        if (spinner != null)

...
import com.zeferino.bruno.pap.adapter.CastDetailsSlideAdapter;
...
import com.zeferino.bruno.pap.model.MovieModel;
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (currentId != this.getArguments().getInt("id") || this.timeOut == 1) {
            ...
            new Thread(new Runnable() {
                public void run() {
                    try {
                        request.execute(Constants.url + "person/" + currentId + "?append_to_response=combined_credits%2Cimages&api_key=" + Constants.key).get(10000, TimeUnit.MILLISECONDS);
                        ...
                        for (int i = 0; i < creditsArray.length(); i++) {
                            ...
                            if (isAdded()) {
                                if (!object.getString("poster_path").equals("null") && !object.getString("poster_path").isEmpty())
                                    movie.setPosterPath(Constants.imageUrl + getResources().getString(R.string.imageSize) + object.getString("poster_path"));
                            }
                        ...
                        for (int i = 0; i < simLen; i++) {
                            ...
                            if (isAdded()) {
                                if (movieListNoDuplicates.get(i).getPosterPath() != null)
                                    simMov.setPosterPath(Constants.imageUrl + getResources().getString(R.string.imageSize) + movieListNoDuplicates.get(i).getPosterPath());
                            }
